// ECS, Task Definition, Security Groups, NLB
...
// this is the ECS cluster that will host our services
// securitygroup is used to control access to the services so that only the NLB can access them. nbl( Network Load Balancer) is used to route traffic to the services
        const dbSecurityGroup = new SecurityGroup(this, 'DbSecurityGroup', {
            vpc,
            allowAllOutbound: true,
        });

        // Allow traffic from the VPC to the database and Kafka services security group. This is necessary because the services will be running in the VPC and need to communicate with each other.
        ...
        // cluster is nothing but its elastic container service cluster that will host our services. we are creating a new cluster here and passing the vpc to it so that the services can communicate with each other.

        //nlb includes kafka and sql as it open sockets for the services to communicate with each other. rather than http, therfore we are not using alb. we are using nlb because it is a layer 4 load balancer and can handle tcp traffic. alb is a layer 7 load balancer and can only handle http traffic. therefore we are using nlb here.
        ...
        // the target group is used to route traffic to the services. we are creating two target groups here, one for the mysql service and one for the kafka service. we are using the same security group for both services because they will be running in the same vpc and need to communicate with each other.
        // taskdefinition is used to define the container that will be running in the service. we are creating two task definitions here, one for the mysql service and one for the kafka service. we are using the same security group for both services because they will be running in the same vpc and need to communicate with each other.
